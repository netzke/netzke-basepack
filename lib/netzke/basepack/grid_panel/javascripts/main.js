{
  ...
  initComponent: function(){
      ...
      if (c.editor && c.editor.xtype == 'checkbox') {
        ...
        if (!c.renderer) {
          ...
          c.renderer = function(value, a, r, ri, ci){
            ...
            if (editor && editor.getStore && editor.getStore().getTotalCount() > 0) {
              return editor.getStore().getById(value).get('name');
            } else if (c.assoc) {
              return r.get('_meta').associationValues[c.name] || value;
            } else {
              return value;
            }
          };
    ...
    Ext.each(this.columns, function(column){
      this.recordConfig.push({name:column.name, defaultValue:column.defaultValue});
    }, this);
    ...
    this.on('beforeedit', function(e){
      ...
      if (column.assoc && column.getEditor().isXType('combo')) {
  ...
  normalizeEditor: function(c) {
    if (c.assoc) {
      // var prefetchedStoreValues = [[0, '']];
      // for (var assocId in this.associationValues[c.name]) {
      //   prefetchedStoreValues.push([parseInt(assocId), this.associationValues[c.name][assocId]]);
      // }
      // c.editor.store = prefetchedStoreValues;
    } else {
      if (c.editor) {
