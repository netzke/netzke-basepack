{
  ...
  onApply: function() {
    if (this.fireEvent('apply', this)) {
      ...
      console.info("applying mask...");
      // commented out on 2011-05-11 because of fatal errors
      // apply mask
      // if (!this.applyMaskCmp) this.applyMaskCmp = new Ext.LoadMask(this.bwrap, this.applyMask);
      // this.applyMaskCmp.show();

      console.info("checking FileUpload..." + this.fileUpload);
  ...
  setFormValues: function(values){
    ...
    for (var assocFieldName in assocValues) {
      ...
      var assocField = this.items.filter('name', assocFieldName).first();
      if (assocField.isXType('combobox')) {
        // WARNING: using private property 'store' here!
