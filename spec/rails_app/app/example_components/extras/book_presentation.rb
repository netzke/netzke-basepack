  ...
  module BookPresentation
    ...
    def author_first_name_setter
      lambda do |r,v|
        ...
        model_adapter = Netzke::Basepack::DataAdapters::AbstractAdapter.adapter_class(Author).new(Author)
        # cast v to integer, if possible
        v = v.to_i if v.kind_of?(String) && v.match(/[[:digit:]]+/)
        if v.is_a?(Integer)
          r.author = model_adapter.find_record(v)
        else
          author = model_adapter.new_record(:first_name => v)
