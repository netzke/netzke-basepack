{
  ...
  initComponent: function(){
    ...
    if (this.paging) {
      this.dockedItems.push({
        ...
        dock: 'bottom',
        listeners: {
          'beforechange': this.disableDirtyPageWarning ? {} : {fn: this.nzBeforePageChange, scope: this}
        },
        store: this.store,
    ...
    this.on('afterlayout', function() {
      ...
      if (this.persistence) {
        ...
        this.on('columnresize', this.nzSaveColumns, this);
        this.on('columnmove', this.nzSaveColumns, this);
        this.on('columnhide', this.nzSaveColumns, this);
        this.on('columnshow', this.nzSaveColumns, this);
      }
    }, this);
    ...
    if(this.nzRememberSelection && this.nzRestoreSelection){
      this.getStore().on('beforeload', this.nzRememberSelection, this);
      this.getView().on('refresh', this.nzRestoreSelection, this);
    }
  },

  nzBuildStore: function() {
    var store = Ext.create('Ext.data.Store', this.nzStoreConfig());
  ...
  nzBuildProxy: function() {
    return Ext.create('Netzke.Basepack.Grid.Proxy', this.nzProxyConfig());
  },

  nzProxyConfig: function(){
    return {
      reader: this.nzBuildReader(),
      grid: this
    }
  },

  nzBuildReader: function() {
