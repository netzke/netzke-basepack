class SomeBorderLayout < Netzke::Basepack::BorderLayoutPanel
  ...
  def configure(c)
    super
    c.items = [
      {:title => "Who", :class_name => "Netzke::Grid::Base", :region => :center, :model => "User", :name => :user_grid},
      {:title => "Item Two", :class_name => "Netzke::Grid::Base", :region => :west, :width => 500, :split => true, :collapsible => true, :model => "Role", :name => :role_grid}
    ]
